...
```{r Project Set-Up and Nutrient Run}
...
pf + scale_fill_manual(values = c('#56B4E9','#D55E00', '#009E73' )) + guides(fill = FALSE)
```


```{r Nutrient Box Plot (nbp)}
...
```{r Nutrient Data of just Possum Creek and Hogtown Upstream (pchu_ts)}
...
```{r Field Parameter Bar Charts (fpb)}
fp = data_fp %>% 
  filter(Date >= as.Date('2018-10-18') & Date <=  as.Date('2019-02-06') ) %>%
...
```{r Field Parameter Time Series (fpts)}
